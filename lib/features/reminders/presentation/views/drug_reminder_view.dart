  ...
  Future<void> _scheduleNotification(DrugReminder reminder) async {
    ...
    if (reminder.drugName.isEmpty) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('يرجى إدخال اسم الدواء')));
      return;
    }
    if (reminder.time.isEmpty) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('يرجى تحديد وقت التذكير')));
      return;
    }
    if (reminder.daysSelected.isEmpty ||
        reminder.daysSelected.every((selected) => !selected)) {
